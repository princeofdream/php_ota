    ...
    while ( $file = readdir ( $dirHandle ) ) {
        ...
        if (strlen ( $file ) > 4) {
            ...
            foreach ( $rootDom as $root ) {
                ...
                // lijun modify versionpath ->getElementsByTagName("path")
                ...
                // lijun scattermd5
                $scattermd5_array = $root->getElementsByTagName ( "scattermd5" );
                $scattermd5_node = $scattermd5_array->item ( 0 );
                if (isset ( $scattermd5_node )) {
                    $scattermd5 = $scattermd5_node->nodeValue;
                }
                // lijun fingerprint
                $fingerprint_array = $root->getElementsByTagName ( "fingerprint" );
                $fingerprint_node = $fingerprint_array->item ( 0 );
                if (isset ( $fingerprint_node )) {
                    $fingerprint = $fingerprint_node->nodeValue;
                }
                // liujie android_version
                $androidVersion_array = $root->getElementsByTagName ( "androidVersion" );
                $androidVersion_node = $androidVersion_array->item ( 0 );
                if (isset ( $androidVersion_node )) {
                    $androidVersion = $androidVersion_node->nodeValue;
                }
                if (! isset ( $build_number ) || strlen ( $build_number ) <= 0) {
                    ...
                    if (! isset ( $version_id ) || $version_id <= 0) {
                        ...
                        if ($affected_nums <= 0) {
                            ...
                            $sql6 = 'insert into version_detail (version_id,version,version_name,version_path,version_size,version_compress,release_notes,version_time,update_time,scattermd5,fingerprint,android_version) values (' . $version_id . ',"' . $build_number . '","' . $version_name . '","' . $version_path . '",' . $image_size . ',' . $image_compress . ',"' . $releasenote . '",' . $version_time . ',' . $update_time . ',"' . $scattermd5 . '","' . $fingerprint . '","' . $androidVersion . '")';
