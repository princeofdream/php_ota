	...
	if($vercode==$illegal_code){
		...
		if(count($array_ver)<5) {
			...
			if(!isset($tb)) {
				...
				$sql2='select delta.delta_id,delta.delta_notes,delta.delta_size, delta.version_id, delta.delta_version from delta where '.$sle2 . ' order by delta_id desc';
                ...
                while($result2=mysql_fetch_array($query2)) {
					...
					$target_version_id = $result2['version_id'];
