...
{
	$check_sn_str = sprintf("SELECT * FROM `perseus_users` WHERE `user`=\"%s\" ",get_sn($info));
	...
	logd("get user from get: " . get_sn($info). " passwd: " . get_passwd($info));

	if( strcmp($row['user'], get_sn($info)) == 0 && strcmp($passwd_from_db, get_passwd($info)) == 0)
	{
		logd("----------------------------Insert info into DB------------------------------------------");
		// $ret = show_data($mdb);
		$encrypt_ret_str = sprintf("/home/james/Environment/web_base/bin/enc common_enc zok_%s",get_sn($info));
...
function update_server_main( $info)
{
	...
	$check_idx_str  = sprintf("SELECT max(idx) FROM `perseus_sn_write_log` WHERE 1");
	$check_idx_stat = mysqli_query($mdb, $check_idx_str);
	$idx_ret = mysqli_fetch_array($check_idx_stat);

	logd("idx max: " . $idx_ret['max(idx)']);
	$idx = intval($idx_ret['max(idx)']);
	$idx = $idx+1;

	$sql_str = sprintf("INSERT INTO `perseus_sn_write_log`(`idx`, `user`, `passwd`, `timestamp`, `session`, `source`) VALUES ('%d','%s','%s',now(),'','')",
		$idx,get_sn($info),get_passwd($info));
	logd($sql_str);
	$sql_ret = mysqli_query($mdb, $sql_str);
	logd($sql_ret);
...
$base_info = array($get_serv, $get_port, $get_remoteip, $get_idx, $get_sn, $get_passwd, $get_session, $get_timestamp, $get_source, $get_show );
